...
void WinTXT::print (int x, int y, char c) {
    ...
    win[y*dimx+x] = c;
    int i = 0;
    while(current_col[i] != '\0')
    {
        col[y*dimx+x][i] = current_col[i];
        i++;
    }
    col[y*dimx+x][i] = '\0';
}
...
void WinTXT::setCol(const char* code) {
    int i = 0;
    while(code[i] != '\0') {
        current_col[i] = code[i];
        i++;
    }
    current_col[i] = '\0';
}
...
void WinTXT::draw (int x, int y) {
    ...
    for(int j=0;j<dimy;++j) {
        for(int i=0;i<dimx;++i) {
            printf("%s%c%s", col[j*dimx+i], win[j*dimx+i], end_col);
        }
        printf("\n");
    }
    termMove(0,dimy);
